...
def generate_analysis(code):
    ...
    try:
        ...
        df_daily = clean(df_d)
        df_min_raw = clean(df_m, is_min=True)
        
        # --- [关键修改点 1] 过滤分时数据，只留“当天” ---
        # 这一行确保分时图只显示最后一个交易日
        ...
        # 核心：今日开盘从日线 df_daily 取，分时图取最后一天现价、最高、最低
        m_o = df_daily['Open'].iloc[-1]               
        m_c = df_min['Close'].iloc[-1]                 
        m_h = df_min['High'].max()                     
        m_l = df_min['Low'].min()                      
...
if params.get("mode") == "api":
    ...
    if data is not None:
        st.json({"status": "success", "stock_name": name, "latest_price": float(data['Close'].iloc[-1])})
    st.stop()
